<template>
  <n-card style = "margin-block: 20px;">
    ...
    <n-button @click="addActivity">新建活动</n-button>
  </n-card>
  ...
  >
    <n-card>
      ...
      <n-h4 style="margin: 5px 0px;">重新上传审核证明</n-h4>
      <div>若审核证明无修改，请勿再次上传</div>
      <n-upload
        :action="uploadURL"
...
async function onPositiveClickDelete(){
  await request("/admin/activity/delete?aid=" + nowAct.value.id, null, "删除活动");
  message.success("已删除");
...
async function onPositiveClickEdit () {
  ...
  nowAct.value.status = "Unreviewed";
  ...
  await request("/admin/activity/update", newAct, "修改活动");
  message.success("修改已上传");
  getList(pagination.page);
  showModalEdit.value = false;
}
